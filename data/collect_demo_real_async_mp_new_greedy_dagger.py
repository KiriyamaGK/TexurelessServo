...
from utils.greedy_dagger import greedy_state_selection
import cv2
import numpy as np
import json
import h5py
...
def frame_sample(episode_data, gap=1):
    ...
    if isinstance(episode_data, dict):
        ...
        # Create new sampled episode data
        if episode_data is not None and len(episode_data) == n:
            # Sample the list using the indices
            sampled_data = [episode_data[i] for i in indices]
        else:
            # For non-list data or data with different length, copy as-is
            sampled_data = episode_data
...
def control_process(config, dagger_config, state, goal_state, is_dagger_episode, collision_result, status_episode, policy_action, subprocess_permit, enable_print,n_base_expert_trajs,cur_fail_pool):
        ...
        while True:
            ...
            if is_dagger_episode.value:
                ...
                if reinit_res["close_enough"] or end_episode:
                    ...
                    pos = None
                    if current_episode - 1 >= n_base_expert_trajs.value:
                        pos = cur_fail_pool.pop(0)
                        env.action_abs_T(env.wgT_tar @ pos)
                    else:
                        env.action_abs_T(env.wgT_tar @ env.g_tar_g_init_T)
                    subprocess_permit.value = False
                    # rubbish_t_start = time.time()
    ...
    try:
        while True:
            ...
            if is_dagger_episode.value:
                ...
                if wgT is None or wgT_tar is None:
                    continue
...
def main_process():
    ...
    global n_base_expert_trajs, cur_fail_pool
    ...
    n_base_expert_trajs = manager.Value('i', 0)  # 整数
    cur_fail_pool = manager.list()  # 可共享的列表
    ...
    # Load configuration
    config_dir = "../configs/demo_collection_real_greedy_dagger.json"
    ...
    is_dagger_episode.value = False #todo:remain modified
    ...
    processes.append(mp.Process(
        ...
        args=(config, dagger_config, state, goal_state, is_dagger_episode, collision_result, status_episode, policy_action, subprocess_permit,enable_print,n_base_expert_trajs,cur_fail_pool),
        name="ControlProcess"
    ))
    processes.append(mp.Process(
    ...
    # pre-defined params
    n_base_expert_trajs.value = dagger_config["n_base_expert_trajs"]
    ...
    try:
        ...
        while episode_count < demo_total_num:
            ...
            if local_new_episode:
                ...
                # ================================dagger===============================
                if is_dagger_episode.value:
                    print("[INFO] Using DAgger strategy for iteration {}".format(episode_count))
                # ================================dagger===============================
            ...
            if not abandon_save_data and all(state.get(k) is not None for k in state.keys()):
                ...
                if not local_new_episode:
                    ...
                    g_tar_T_g = np.linalg.inv(state['wgT_tar'][:]) @ state['wgT'][:]
                    g_tar_T_g_list.append(g_tar_T_g)

                    # ============================DAgger prediction step============================
                    if is_dagger_episode.value and policy_model:
            ...
            if epi_length[episode_count] > 0 and len(episode_data["obs"]) == epi_length[episode_count]:
                ...
                if is_dagger_episode.value:
                    ...
                    policy_actions_np = [np.array(action[:]) for action in policy_action_list]
                    # frame sample
                    g_tar_T_g_list = frame_sample(g_tar_T_g_list,frame_sampling_gap)
                    policy_actions_np = frame_sample(policy_actions_np,frame_sampling_gap)

                    selected_states = greedy_state_selection(g_tar_T_g_list, policy_actions_np,
                                                             episode_data['expert_actions'],
                                                             num_selected_pts, w1, w2, s_tr, s_rot, a_tr, a_rot,
                                                             action_error_threshold, is_mm=True)
                    rest_fail_pool_size += len(selected_states)
                    total_fail_pool_size += len(selected_states)
                    for _state in selected_states:
                        cur_fail_pool.append(_state)
                else:
                    num_expert_trajs += 1
                    if episode_count >= n_base_expert_trajs.value:
                        rest_fail_pool_size -= 1

                should_train = (total_fail_pool_size >= n_fail_pool_size and rest_fail_pool_size == 0 and not is_dagger_episode.value) or episode_count == n_base_expert_trajs.value -1
                if should_train and policy_model is not None:
                    ...
                    f_tmp.close()
                    create_hdf5_filter_key(hdf5_path=dataset_dir, demo_keys=all_demos, key_name=filter_key,
                                           return_length=False)

                    # Train policy with filter key
                ...
                if episode_count < n_base_expert_trajs.value - 1:
                    is_dagger_episode.value = False
                elif episode_count == n_base_expert_trajs.value - 1:
                    is_dagger_episode.value = True
                else:
                    if is_dagger_episode.value:
                        if total_fail_pool_size >= n_fail_pool_size:
                            is_dagger_episode.value = False
                            print(f"[DAgger] Policy switched form dagger to expert in iteration {episode_count}.")
                    if should_train and episode_count >= n_base_expert_trajs.value:
                        is_dagger_episode.value = True
                        total_fail_pool_size = 0  # rest_fail_pool_size is set 0 above
                        assert len(cur_fail_pool) == 0
                        print(f"[DAgger] Policy switched from expert to dagger in iteration {episode_count}.")

                # new iteration setting is in ctrl process
                print("total_fail_pool_size:",total_fail_pool_size)
                print("rest_fail_pool_size:", rest_fail_pool_size)
                # ================================dagger===============================
                if is_dagger_episode.value:
                    print(f"[DAgger] Iteration {episode_count} completed successfully")
                # ================================dagger===============================
