...
class HDF5Dataset(Dataset):
    def __init__(self, hdf5_path, seq_length: int = 1, specific_obs_keys: list = None, bgr2rgb: bool = False, num_demos: int = None):
        ...
        self.file = h5py.File(hdf5_path, "r", rdcc_nbytes=1024*1024*1024*20)  # 20GB cache

        episode_keys = list(self.file["data"].keys())
        ...
        self.episode_keys = [pair[1] for pair in sorted_pairs][0:self.num_demos]

        self.episode_lens = [len(self.file["data/{}/actions".format(ep)]) for ep in self.episode_keys]
