...
def plot_rot_and_trans(error_rot_lst,error_trans_lst,z_error_lst,use_time=10,obj_pth=None,show=False):
    ...
    # Create time axis
    time_axis = np.linspace(0, use_time, len(error_rot_lst))

    ax1.plot(time_axis, error_rot_lst, label='Rotation Error', marker='o', linestyle='-', color='blue')
    ...
    ax2.plot(time_axis, error_trans_lst, label='Trans XYZ Error', marker='x', linestyle='--', color='red')
    ...
    if dof == 6:
        ax3.plot(time_axis, z_error_lst, label='Trans Z Error', marker='x', linestyle='--', color='green')
    ...
    # Final error annotations
    final_rot_error = error_rot_lst[-1]
    final_trans_error = error_trans_lst[-1]

    ax1.annotate(f'Final: {final_rot_error:.2f}°',
                 xy=(time_axis[-1], final_rot_error),
                 xytext=(time_axis[-1], final_rot_error + 0.05),
                 color='blue')

    ax2.annotate(f'Final: {final_trans_error:.2f}mm',
                 xy=(time_axis[-1], final_trans_error),
                 xytext=(time_axis[-1], final_trans_error + 0.05),
                 color='red')

    if dof == 6:
        final_z_error = z_error_lst[-1]
        ax3.annotate(f'Final: {final_z_error:.2f}mm',
                     xy=(time_axis[-1], final_z_error),
                     xytext=(time_axis[-1], final_z_error + 0.05),
                     color='green')
...
def plot_error_pose(error_pos_list, use_time, obj_pth, show):
    ...
    # Create time axis
    time_axis = np.linspace(0, use_time, len(error_pos_list))

    ax1.plot(time_axis, error_pos_list[:, 2], label='Pose Estimation Rotation Error', marker='o', linestyle='-',
             color='blue')
    ax1.set_title('Rotation Error')
    ax1.set_xlabel('Time (s)')
    ax1.set_ylabel('Error(°)')
    ...
    ax2.plot(time_axis, error_pos_list[:, 0], label='Pose Estimation Trans XYZ Error', marker='x', linestyle='--',
             color='red')
    ax2.set_title('Translation XYZ Error')
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Error(mm)')
    ...
    ax3.plot(time_axis, error_pos_list[:, 1], label='Pose Estimation Trans Z Error', marker='x', linestyle='--',
             color='green')
    ax3.set_title('Translation Z Error')
    ax3.set_xlabel('Time (s)')
    ax3.set_ylabel('Error(mm)')
...
def plot_trajs(wgT_list, wgT_tar, motion_type, obj_path=None, show=False, is_mm=False):
    wgT_list = np.array(wgT_list)
    assert len(wgT_list.shape) == 3, "wgT_list must be a 3D array"

    tar_pos = wgT_tar[0:3, 3] if is_mm else wgT_tar[0:3, 3] * 1000
    tar_rot = R.from_matrix(wgT_tar[0:3, 0:3]).as_rotvec() * 180 / np.pi
    ...
    for wgT in wgT_list:
        ...
        if axis_angle[0] > 0:
            ori_norm = np.linalg.norm(axis_angle[0:3])
            axis_angle[0:3] *= -(360 - ori_norm) / ori_norm
    ...
    # Use time steps as x-axis (assuming uniform time distribution)
    ...
    axes[1, 1].set_ylabel('Rotation (deg)')
    axes[1, 1].grid(True)
    axes[1, 1].legend()
    ...
    axes[2, 0].set_ylabel('Rotation (deg)')
    axes[2, 0].grid(True)
    axes[2, 0].legend()
    ...
    axes[2, 1].set_ylabel('Rotation (deg)')
    axes[2, 1].grid(True)
    axes[2, 1].legend()
...
def plot_vel(vel_tr, vel_rot, use_time, obj_path=None, show=False):
    ...
    # Create time axis
    time_axis = np.linspace(0, use_time, len(vel_rot))

    ax1.plot(time_axis, vel_rot, label='Rotation Velocity', marker='o', linestyle='-', color='blue')
    ...
    ax2.plot(time_axis, vel_tr, label='Translation Velocity', marker='x', linestyle='--', color='red')
...
def plot_6dvel(vel, use_time, obj_path=None, show=False):
    ...
    # 旋转速度（3个分量）
    ax1.plot(time_axis, vel_rot[:, 0], label='theta1', linestyle='-', color='blue')
    ax1.plot(time_axis, vel_rot[:, 1], label='theta2', linestyle='-', color='green')
    ax1.plot(time_axis, vel_rot[:, 2], label='theta3', linestyle='-', color='red')
    ...
    # 平移速度（3个分量）
    ax2.plot(time_axis, vel_tr[:, 0], label='X', linestyle='-', color='blue')
    ax2.plot(time_axis, vel_tr[:, 1], label='Y', linestyle='-', color='green')
    ax2.plot(time_axis, vel_tr[:, 2], label='Z', linestyle='-', color='red')
...
def plot_img_diff(diff_list, use_time, obj_path=None, show=False):
    ...
    # Create time axis
    time_axis = np.linspace(0, use_time, len(diff_list))

    plt.plot(time_axis, diff_list, label='Difference', marker='o', linestyle='-')
    plt.xlabel('Time (s)')
    ...
    min_points = [(time_axis[idx], diff_list[idx]) for idx in sorted_indices]

    # 准备表格数据
    table_data = [['Time (s)', 'Difference']]
