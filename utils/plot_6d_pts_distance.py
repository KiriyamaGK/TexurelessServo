...
def plot_6d_pts(X: np.ndarray, dim=2, method=None, color_list=None):
    """
    可视化6维向量，支持按颜色分类画轨迹
...
if __name__ == "__main__":
    ...
    hdf_pth = "/media/kiriyamagk/One Touch/AlignAnything/25.10.16/hdf5/mimic.hdf5"
    ...
    traj_vis_gap = 3
    # traj_vis_type = "all" # "all" or [(opt)"base_expert", (opt)"dagger", (opt)"aggregated_expert"]
    traj_vis_type = ["dagger", "aggregated_expert"] # "all" or [(opt)"base_expert", (opt)"dagger", (opt)"aggregated_expert"]
    # traj_vis_type = ["dagger"]  # "all" or [(opt)"base_expert", (opt)"dagger", (opt)"aggregated_expert"]
    ...
    plot_6d_pts(X,dim=vis_dim,method = vis_method,color_list = color_list)
