...
class TransformerCenternet(NetworkBase):
    def __init__(self, input_low_dim, output_dim, obs_keys,batch_size,seq_length,training,embedding_size=656,n_layer=4,n_head=4,block_size=10,low_dim_hidden_sizes=None,output_head_sizes=None,activation="relu", output_activation=None,use_gmm=False,
        ...
        if not self.use_siamese:
            self.deconv_layers_goal = nn.Sequential(
                nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1, bias=False),
            ...
            # 输出头：预测中心点热图、偏移量和物体尺寸
            self.head_heatmap_goal = nn.Sequential(
                nn.Conv2d(64, 64, kernel_size=3, padding=1, bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
                nn.Conv2d(64, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x_kpt_goal = x['gaussian_img_kpt_goal'].to(self.device)
        x_kpt_goal = x_kpt_goal.view(-1, 1, self.img_size // 4, self.img_size // 4)
        ...
        heatmap_goal = self.deconv_layers_goal(x_img_goal.clone())
        heatmap_goal = self.head_heatmap_goal(heatmap_goal)
        heatmap_goal = heatmap_goal.view(-1, 1, self.img_size // 4, self.img_size // 4)
        # img_ln
        ...
        if not self.use_GMM:
            ...
            "pred_heatmap":torch.concatenate((heatmap,heatmap_goal), dim=1),
            "kpt_heatmap_gt":torch.concatenate((x_kpt,x_kpt_goal), dim=1),
            }
        return rtn_dict